...
def pil_to_normalized_tensor(image: Image.Image, image_size: int) -> torch.Tensor:
    ...
    return data.mul(2.0).sub(1.0)


class PromptedCIFAR10(Dataset):
    def __init__(self, root: str, image_size: int, train: bool = True, prompt_template: str = "a photo of a {}"):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        return {
            "image": pil_to_normalized_tensor(image, self.image_size),
            "label": label,
            "prompt": self.prompt_template.format(class_name),
        }


class PromptedImageFolder(Dataset):
    def __init__(
        self,
        root: str,
        image_size: int,
        ...
        split_path = root_path / str(split) if split else root_path
        data_path = split_path if split_path.exists() else root_path
        if not data_path.exists():
            raise FileNotFoundError(f"ImageNet/ImageFolder path not found: {data_path}")

        self.dataset = ImageFolder(root=str(data_path))
        if len(self.dataset) == 0:
            raise RuntimeError(f"No images found under ImageFolder path: {data_path}")
        self.image_size = image_size
        self.prompt_template = prompt_template
        self.classes = [imagenet_prompt_name(name) for name in self.dataset.classes]
        self.indices = self._build_indices(max_classes=max_classes, max_samples_per_class=max_samples_per_class)

    def _build_indices(self, max_classes: int | None, max_samples_per_class: int | None) -> list[int]:
        allowed_classes = set(range(len(self.dataset.classes)))
        if max_classes is not None and max_classes > 0:
            allowed_classes = set(range(min(max_classes, len(self.dataset.classes))))
        counts: dict[int, int] = {}
        indices = []
        for index, (_, class_index) in enumerate(self.dataset.samples):
            if class_index not in allowed_classes:
                continue
            count = counts.get(class_index, 0)
            if max_samples_per_class is not None and max_samples_per_class > 0 and count >= max_samples_per_class:
                continue
            indices.append(index)
            counts[class_index] = count + 1
        if not indices:
            raise RuntimeError("ImageNet subset selection is empty; check max_classes/max_samples_per_class")
        return indices
...
class PromptedCocoCaptions(Dataset):
    def __init__(
        self,
        root: str,
        image_size: int,
        ...
        caption_choice: str = "random",
        skip_missing_images: bool = False,
    ):
        root_path = Path(root).expanduser()
        split_name = str(split or "train")
        coco_split = split_name if any(char.isdigit() for char in split_name) else f"{split_name}2017"

        if annotation_file:
            annotation_path = Path(annotation_file).expanduser()
            if not annotation_path.is_absolute():
                annotation_path = root_path / annotation_path
        else:
            annotation_path = root_path / "annotations" / f"captions_{coco_split}.json"

        if image_dir:
            image_dir_path = Path(image_dir).expanduser()
            if not image_dir_path.is_absolute():
                image_dir_path = root_path / image_dir_path
        else:
            image_dir_path = root_path / coco_split

        if not annotation_path.exists():
            raise FileNotFoundError(f"COCO captions annotation file not found: {annotation_path}")
        if not image_dir_path.exists():
            raise FileNotFoundError(f"COCO image directory not found: {image_dir_path}")
        ...
        for image_id, image in images_by_id.items():
            ...
            if not captions:
                continue
            image_path = image_dir_path / str(image["file_name"])
            if skip_missing_images and not image_path.exists():
                continue
            entries.append((image_path, image_id, captions))
        if not entries:
            raise RuntimeError(f"No captioned COCO images found in {annotation_path}")

        self.entries = entries
        self.image_size = image_size
        self.prompt_template = prompt_template
        self.caption_choice = caption_choice

    def __len__(self) -> int:
        return len(self.entries)

    def _select_caption(self, captions: list[str], idx: int) -> str:
        if self.caption_choice == "first":
            return captions[0]
        if self.caption_choice == "cycle":
            return captions[idx % len(captions)]
        if self.caption_choice == "random":
            return random.choice(captions)
        raise ValueError(f"Unsupported caption_choice: {self.caption_choice}")

    def __getitem__(self, idx: int) -> dict[str, Any]:
        path, image_id, captions = self.entries[idx]
        caption = self._select_caption(captions, idx)
        with Image.open(path) as image:
            tensor = pil_to_normalized_tensor(image, self.image_size)
        return {
            "image": tensor,
            "label": image_id,
            "prompt": self.prompt_template.format(caption),
        }


class PromptedGenEvalImages(Dataset):
    """Generated images paired with Flow-GRPO GenEval metadata."""
    ...
    def __init__(
        ...
        split: str | None = "train",
        code_pool: str | None = None,
        target_root: str | None = None,
    ):
        self.root = Path(root).expanduser()
        split_name = str(split or "train")
        metadata_path = Path(metadata_file).expanduser() if metadata_file else self.root / f"{split_name}_metadata.jsonl"
        ...
        with metadata_path.open("r", encoding="utf-8") as f:
            for line_number, line in enumerate(f, start=1):
                ...
                image_value = metadata.get("image_path") or metadata.get("file_name") or metadata.get("image")
                if not prompt or not image_value:
                    raise ValueError(
                        f"GenEval record {metadata_path}:{line_number} requires prompt and image_path/file_name/image"
                    )
                image_path = Path(str(image_value)).expanduser()
                if not image_path.is_absolute():
                    image_path = self.root / image_path
                if not image_path.exists():
                    raise FileNotFoundError(f"GenEval image not found: {image_path}")
                entries.append((image_path, prompt, metadata))
        if not entries:
            raise RuntimeError(f"No GenEval image records found in {metadata_path}")

        self.entries = entries
        self.image_size = int(image_size)
        self.codes: torch.Tensor | None = None
        self.target_root = Path(target_root).expanduser() if target_root else None
        if code_pool:
            pool_path = Path(code_pool).expanduser()
            if not pool_path.exists():
                raise FileNotFoundError(f"GenEval LlamaGen code pool not found: {pool_path}")
            pool = torch.load(pool_path, map_location="cpu", weights_only=False)
            if not isinstance(pool, dict) or not torch.is_tensor(pool.get("codes")):
                raise RuntimeError(f"Expected a code pool with tensor field 'codes': {pool_path}")
            codes = pool["codes"].long()
            if codes.ndim != 3:
                raise RuntimeError(f"Expected GenEval codes with shape [N,H,W], got {tuple(codes.shape)}")
            max_index = max(
                int(metadata.get("llamagen_sample_index", index))
                for index, (_, _, metadata) in enumerate(entries)
            )
            if max_index >= len(codes):
                raise RuntimeError(f"GenEval manifest references code {max_index}, but pool has {len(codes)} codes")
            self.codes = codes
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        image_path, prompt, metadata = self.entries[idx]
        ...
        if self.codes is not None:
            code_index = int(metadata.get("llamagen_sample_index", idx))
            sample["llamagen_code"] = self.codes[code_index]
            if self.target_root is not None:
                target_path = self.target_root / "images" / f"{code_index:06d}.png"
                if not target_path.exists():
                    raise FileNotFoundError(f"GenEval offline target not found: {target_path}")
                with Image.open(target_path) as target:
                    sample["geneval_target"] = pil_to_normalized_tensor(target, self.image_size)
        return sample
...
class SyntheticShapesDataset(Dataset):
    ...
    def __init__(self, length: int, image_size: int, prompt_template: str = "a simple {}"):
        self.length = length
        self.image_size = image_size
        self.prompt_template = prompt_template
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if class_name == "circle":
            ...
            draw.polygon([(size // 2, pad), (size - pad, size - pad), (pad, size - pad)], fill=fg)
        else:
            width = max(2, math.ceil(size / 8))
            draw.line((pad, pad, size - pad, size - pad), fill=fg, width=width)
            draw.line((size - pad, pad, pad, size - pad), fill=fg, width=width)

        return {
            "image": pil_to_normalized_tensor(image, size),
            "label": label,
            "prompt": self.prompt_template.format(class_name),
        }


def imagenet_prompt_name(folder_name: str) -> str:
    name = folder_name.replace("_", " ").strip()
    if name.startswith("n") and len(name) >= 9 and name[1:9].isdigit():
        index = IMAGENET_SYNSET_TO_INDEX.get(name[:9])
        if index is not None:
            return _IMAGENET_CATEGORIES[index]
        return name
    return name


IMAGENET_SYNSETS = (
    "n01440764", "n01443537", "n01484850", "n01491361", "n01494475", "n01496331", "n01498041", "n01514668",
...
IMAGENET_SYNSET_TO_INDEX = {synset: index for index, synset in enumerate(IMAGENET_SYNSETS)}
...
def build_dataloader(cfg: Any, reward_cfg: Any, batch_size: int, distributed: bool) -> LoaderBundle:
    prompt_template = getattr(reward_cfg, "prompt_template", "a photo of a {}")
    dataset_name = cfg.dataset
    if dataset_name == "cifar10":
        dataset: Dataset = PromptedCIFAR10(
            root=str(Path(cfg.root).expanduser()),
            image_size=int(cfg.image_size),
            train=True,
            prompt_template=prompt_template,
        )
    elif dataset_name in {"imagenet", "imagenet_subset", "imagefolder"}:
        dataset = PromptedImageFolder(
            root=str(Path(cfg.root).expanduser()),
            image_size=int(cfg.image_size),
            prompt_template=prompt_template,
            split=getattr(cfg, "split", "train"),
            max_classes=getattr(cfg, "max_classes", None),
            max_samples_per_class=getattr(cfg, "max_samples_per_class", None),
        )
    elif dataset_name in {"coco", "coco_captions"}:
        dataset = PromptedCocoCaptions(
            root=str(Path(cfg.root).expanduser()),
            image_size=int(cfg.image_size),
            prompt_template=prompt_template,
            split=getattr(cfg, "split", "train"),
            annotation_file=getattr(cfg, "annotation_file", None),
            image_dir=getattr(cfg, "image_dir", None),
            caption_choice=str(getattr(cfg, "caption_choice", "random")),
            skip_missing_images=bool(getattr(cfg, "skip_missing_images", False)),
        )
    elif dataset_name in {"geneval", "geneval_images"}:
        dataset = PromptedGenEvalImages(
            root=str(Path(cfg.root).expanduser()),
            image_size=int(cfg.image_size),
            metadata_file=getattr(cfg, "metadata_file", None),
            split=getattr(cfg, "split", "train"),
            code_pool=getattr(cfg, "code_pool", None),
            target_root=getattr(cfg, "target_root", None),
        )
    elif dataset_name == "synthetic_shapes":
        length = int(cfg.train_subset or 1024)
        dataset = SyntheticShapesDataset(length=length, image_size=int(cfg.image_size), prompt_template=prompt_template)
    else:
        raise ValueError(f"Unsupported dataset: {dataset_name}")

    if cfg.train_subset is not None and dataset_name not in {"synthetic_shapes", "imagenet", "imagenet_subset", "imagefolder"}:
        dataset = Subset(dataset, range(min(int(cfg.train_subset), len(dataset))))
    if cfg.train_subset is not None and dataset_name in {"imagenet", "imagenet_subset", "imagefolder"}:
        dataset.indices = dataset.indices[: min(int(cfg.train_subset), len(dataset.indices))]
    ...
    sampler = (
        DistributedSampler(
            ...
            shuffle=True,
            drop_last=True,
            seed=int(getattr(cfg, "sampler_seed", 0)),
        )
        if distributed
        else None
    )
    ...
    return LoaderBundle(dataloader=dataloader, sampler=sampler)
